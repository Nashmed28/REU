...
//// EVERYTHING ABOVE HERE IS MY PREV VERSION
...
function parameter_fields (variable, type_chosen) {
    ...
    for (j = 0; j < needed_parameters.length; j++) {
        // creates html list in .sort() (alphabet order)
        parameter_field += "<span title='" + rfunctions.parameter_info[(column_index[needed_parameters[j].replace(/\s/g, '_')] - (4 * type_list.length) - 1)].pinfo + "'>" + needed_parameters[j] + ":</span> <input type='text' value='" + inputted_metadata[variable][column_index[needed_parameters[j].replace(/\s/g, '_')]] + "' name='" + needed_parameters[j].replace(/\s/g, '_') + "'id='input_" + needed_parameters[j].replace(/\s/g, '_') + "_" + variable + "' onclick='record_table()' oninput='Parameter_Memory(this,\"" + variable + "\")' onfocusout='ValidateInput(this, \"" + rfunctions.parameter_info[(column_index[needed_parameters[j].replace(/\s/g, '_')] - (4 * type_list.length) - 1)].entry_type + "\", \"" + variable + "\"); pass_to_r_metadata(this, \"" + variable + "\");'><br>"
    };
...
//                      console.log("talk to r");
                        ...
                        "</td>" +
                        "<td>" +
                            "<input type='text' value='" + inputted_metadata[varlist_active[n].replace(/\s/g, '_')][stat_index + 2] + "' name='accuracy_" + statistic_list[m] + "' onclick='record_table()' onfocusout='ValidateInput(this, \"pos_number\", \"" + varlist_active[n].replace(/\s/g, '_') + "\"); pass_to_r_epsilon();' oninput='Parameter_Memory(this,\"" + varlist_active[n].replace(/\s/g, '_') + "\")'>" +
                        "</td>" +
                        "<td>";
...
function pass_to_r_epsilon () {
//  alert(input.value + ", " + variable + ", " + statistic);
//  talktoR(previous_inputted_metadata,inputted_metadata, column_index);
//  change needs to be detect 
    console.log("talk to r");
};
